...
export function buildMainPanel(): PanelResult {
  ...
  const lines = [
    ...
    '',
    `\u{23F1} Rate: ${config.rateBudget}t/5min (\u{6BCF}\u{53F0} ${perRemoteRateBudget(config)}t)`,
    `\u{1F4C5} Weekly: ${config.weeklyBudget}t/wk (\u{6BCF}\u{53F0} ${perRemoteWeeklyBudget(config)}t)`,
